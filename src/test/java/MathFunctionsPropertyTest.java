...
public class MathFunctionsPropertyTest {
    ...
    @Property
    void dobroSempreMaiorOuIgual(@ForAll @IntRange(min = 0,  max = 1073741823) int n) {
        int resultado = MathFunctions.multiplyByTwo(n);
        assertThat(resultado).isGreaterThanOrEqualTo(n);
    }

    @Property
    void dobroEhIgualASomaDeleMesmo(
    ...
    @Property
    void dobroDeNumeroParTambemEhPar(@ForAll @IntRange(min = -100000, max = 100000) int n) {
        int x = n * 2;
    ...
    @Property
    void multiplyByTwoShouldAlwaysBeEven(@ForAll int number) {
    ...
    @Property
    void allElementsShouldBeMultiplesOfNumber(
    ...
    @Property
    void primeNumbersHaveNoDivisorsExcept1AndItself(@ForAll @IntRange(min = 2, max = 10000) int number) {
    ...
    @Property
    void averageShouldBeBetweenMinAndMax(@ForAll int[] numbers) {
